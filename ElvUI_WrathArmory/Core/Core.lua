...
local dropdown = _G["PlayerStatFrameLeftDropdown"]  -- 使用全小写名称
if dropdown then
    dropdown:ClearAllPoints()
    dropdown:Point('TOP', MagicResFrame3, 'BOTTOM', isSkinned and -5 or 0, -15)
    if dropdown.Middle then
        UIDropDownMenu_SetWidth(dropdown, isSkinned and 140 or 170)
    else
        dropdown:SetWidth(isSkinned and 140 or 170)
        -- 移除提示：print("|cffFF0000[ElvUI_WrathArmory] 提示：PlayerStatFrameLeftDropdown 不是标准下拉菜单，已手动设置宽度。")
    end
else
    print("|cffFF0000[ElvUI_WrathArmory] 警告：未找到 PlayerStatFrameLeftDropdown，可能与当前版本不兼容。")
end

PlayerStatLeftTop:ClearAllPoints()
PlayerStatLeftTop:Point('TOP', dropdown or MagicResFrame3, 'BOTTOM', 0, 8)
...
PlayerStatFrameLeft1:Point('TOPLEFT', PlayerStatLeftTop, 'TOPLEFT', 6, -10)

--* Right Stats Group anchors below Left Stats Group
local rightDropdown = _G["PlayerStatFrameRightDropdown"]  -- 使用全小写名称
if rightDropdown then
    rightDropdown:ClearAllPoints()
    rightDropdown:Point('CENTER', PlayerStatLeftBottom, 'CENTER', 0, -35)
    if rightDropdown.Middle then
        UIDropDownMenu_SetWidth(rightDropdown, isSkinned and 140 or 170)
    else
        rightDropdown:SetWidth(isSkinned and 140 or 170)
        -- 移除提示：print("|cffFF0000[ElvUI_WrathArmory] 提示：PlayerStatFrameRightDropdown 不是标准下拉菜单，已手动设置宽度。")
    end
else
    print("|cffFF0000[ElvUI_WrathArmory] 警告：未找到 PlayerStatFrameRightDropdown，可能与当前版本不兼容。")
end

PlayerStatRightTop:ClearAllPoints()
PlayerStatRightTop:Point('TOP', rightDropdown or PlayerStatLeftBottom, 'BOTTOM', 0, 8)
...
PlayerStatFrameRight1:ClearAllPoints()  -- 新增：调整右侧第一个条目位置
PlayerStatFrameRight1:Point('TOPLEFT', PlayerStatRightTop, 'TOPLEFT', 6, -10)  -- 新增：向下移动
