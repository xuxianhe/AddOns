...
L["/wa pstart - Start profiling. Optionally include a duration in seconds after which profiling automatically stops. To profile the next combat/encounter, pass a \"combat\" or \"encounter\" argument."] = "/wa pstart - 프로파일링을 시작합니다. 프로파일링이 자동으로 중지되는 시간(초)을 옵션으로 넣을 수 있습니다. 다음 전투(combat)/보스 전투(encounter)를 프로파일링하려면 인수로 \"combat\"이나 \"encounter\"를 넣으면 됩니다."
...
|cffffff00*|r Yellow Unit settings will create clones for each matching unit while this trigger is providing Dynamic Info to the Aura.]=] ] = [=[• |cff00ff00플레이어|r, |cff00ff00대상|r, |cff00ff00주시대상|r 및 |cff00ff00소환수|r는 이들의 유닛ID와 직접 대응합니다.

• |cff00ff00특정 유닛|r엔 추적하려는 특정 유닛ID를 입력할 수 있습니다.

|cffff0000참고|r: 게임에선 모든 유닛ID에 대해 이벤트를 발생시키지 않으므로 이 활성 조건으로 일부는 추적할 수 없습니다.

• |cffffff00파티|r, |cffffff00공격대|r, |cffffff00우두머리|r, |cffffff00투기장|r 및 |cffffff00이름표|r는 해당 유닛ID는 여러개의 유닛 ID와 일치할 수 있습니다.

• |cffffff00스마트 그룹|r은 현재 그룹 종류에 맞게 조정되어 그룹에 없을땐 "player"만, 파티에서는 "party" 유닛 ("player" 포함), 공격대에서는 "raid" 유닛과 일치합니다.


|cffffff00*|r 노란색인 유닛 설정은 이 활성 조건이 위크오라에 동적 정보를 제공시 일치하는 각 유닛마다 복제 위크오라를 생성합니다.]=]
...
L["Alive"] = "생존"
L["All"] = "모두"
L["All children of this aura will also not be loaded, to minimize the chance of further corruption."] = "이 위크오라의 모든 자식들도 불러오지 않을 것입니다. 오염 가능성을 최소화하기 위함입니다."
...
L["Aura Broken"] = "오라 해제됨"
L["Aura Broken Spell"] = "주문으로 오라 해제됨"
...
L["Crushing"] = "강타"
L["C'thun"] = "쑨"
L["Cumulated time used during profiling"] = "프로파일링 중에는 축적된 시간이 사용됩니다"
L["Currency"] = "화폐"
...
L["Empower Cast Interrupt"] = "강화 시전 방해"
...
https://github.com/WeakAuras/WeakAuras2/wiki/Protected-Frames]=] ] = [=['%s'|1이;가; 보안 복제품을 생성해서 오류가 발생했습니다. 이 위크오라는 삭제할 것을 권장합니다. 자세한 정보:
https://github.com/WeakAuras/WeakAuras2/wiki/Protected-Frames]=]
...
L["Note: Due to how complicated the swing timer behavior is and the lack of APIs from Blizzard, results are inaccurate in edge cases."] = "참고: 스윙 형태 타이머 동작 방식의 난해함과 Blizzard가 제공하는 관련 API가 없는 관계로 정확도가 떨어지는 극단적인 경우가 있습니다."
...
L["Note: This trigger relies on the WoW API, which returns incorrect information in some cases."] = "참고: 이 활성 조건은 WoW API에 의존하고 있는 관계로 경우에 따라 부정확한 정보를 반환할 수도 있습니다."
L["Note: This trigger type estimates the range to the hitbox of a unit. The actual range of friendly players is usually 3 yards more than the estimate. Range checking capabilities depend on your current class and known abilities as well as the type of unit being checked. Some of the ranges may also not work with certain NPCs.|n|n|cFFAAFFAAFriendly Units:|r %s|n|cFFFFAAAAHarmful Units:|r %s|n|cFFAAAAFFMiscellanous Units:|r %s"] = "참고: 이 활성 조건 종류는 유닛의 히트박스 거리를 계산하고 있습니다. 실제 우호적 플레이어의 거리는 일반적으로 계산보다 3미터 더 떨어져 있습니다. 거리 검사는 당신의 현재 클래스와 습득한 스킬, 검사중인 유닛의 종류에 따라 성능이 달라집니다. 특정 NPC에는 일부 거리 검사가 작동하지 않을 수도 있습니다.|n|n|cFFAAFFAA우호적 유닛:|r %s|n|cFFFFAAAA적대적 유닛:|r %s|n|cFFAAAAFF기타 유닛:|r %s"
...
https://github.com/WeakAuras/WeakAuras2/wiki/Protected-Frames]=] ] = [=[보안 프레임이 감지됨. 자세한 정보:
https://github.com/WeakAuras/WeakAuras2/wiki/Protected-Frames]=]
...
L["Your next encounter will automatically be profiled."] = "다음 보스전때 자동으로 프로파일링됩니다."
