...
instancedifficulty:SetScript("OnEvent", function()
	...
	if not garrisons[mapID] then
		if AllowedInstances[instanceType] then
			if E.db.ElvUI_EltreumUI.skins.instances.enable and not garrisons[mapID] then
				...
				if _G["MiniMapInstanceDifficulty"] and not (_G["MiniMapInstanceDifficulty"]:IsShown() and _G["MiniMapInstanceDifficulty"]:GetAlpha() == 1) then
					_G["MiniMapInstanceDifficulty"]:Show()
					_G["MiniMapInstanceDifficulty"]:SetAlpha(1)
				end
			end
		end
	else
