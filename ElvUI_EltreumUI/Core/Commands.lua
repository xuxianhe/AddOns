...
function ElvUI_EltreumUI:LoadCommands()
	...
	if not self.ConfigModeAddedEltruism then
		E:ConfigMode_AddGroup("ELTREUMUI",E:TextGradient("Eltruism", 0.50, 0.70, 1, 0.67, 0.95, 1))
...
function ElvUI_EltreumUI:RunCommands(message)
	...
	elseif message == "encountercheck" then
		if E.db.ElvUI_EltreumUI.encountercheck then
			...
			ElvUI_EltreumUI:Print("Enabled Encounter Check. Some functions that could improve performance will be disabled during boss fights")
	...
	elseif message == 'screenshotcomparison' then
		...
		E:Delay(0, function()
			SetCVar('screenshotSizeOverride', "3840x2160")
			Screenshot()
		end)
		E:Delay(1, function()
			SetCVar('screenshotSizeOverride', "2560x1440")
			Screenshot()
		end)
		E:Delay(2, function()
			SetCVar('screenshotSizeOverride', "1280x720")
			Screenshot()
		end)
		E:Delay(3, function()
			SetCVar('screenshotSizeOverride', "1920x1080")
			Screenshot()
		end)
		E:Delay(4, function()
			SetCVar('screenshotSizeOverride', "0x0")
			Screenshot()
			ElvUI_EltreumUI:Print("Native, 720p, 1080p, 1440p and 4K screenshots taken")
		end)
	else
