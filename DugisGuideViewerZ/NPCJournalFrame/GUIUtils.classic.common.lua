...
function GUIUtils:CreateHintFrame(x, y, width, height, hintTexture)
    ...
    local frame = CreateFrame("Frame", "DragrFrame2", UIParent, "BackdropTemplate")
    ...
    local texture = window.imageFrame:CreateTexture(nil,"ARTWORK")
    ...
    window.SetModel = function (self, model, modelName, npcId)
        ...
        if model then
            GUIUtils.setCreature(self.modelFrame, npcId or model)
        elseif npcId then
			GUIUtils.setCreature(self.modelFram, npcId)
		else
...
, noScrollMode, columnWidth, nodeTextX, nodeTextY, isInThread)
    ...
    if wrapper == nil then
        ...
        if not wrapper then
            wrapper = CreateFrame("Frame", treePrefix.."wrapper", targetTreeFrame, "BackdropTemplate")
        end
    ...
    for _, visualNode in pairs(treeVisualizationContainer[treePrefix]) do
        visualNode:Hide()
    ...
    for i, nodeData in pairs(nodes) do
		LuaUtils:RestIfNeeded(isInThread)
        if nodeData.isLeaf then
            ...
            nodeData.visualNode = visualNode
            ...
            visualNode.Title:SetFontObject(GameFontNormalSmall)
    ...
    local function UpdateSubTree(visualNodes, visualParentNode, currentYOffset, wrapper, level, columnDeltaX, noScrollMode, columnWidth, isInThread)
        ...
        for index, visualNode in pairs(visualNodes) do
			LuaUtils:RestIfNeeded(isInThread)
			
            local x = 15 * level + columnDeltaX

            if visualNode.nodeData.isLeaf then
                if visualNode.nodeData.noHightlight then
                    visualNode.Button.highlight:Hide()
                end

                if visualParentNode == nil or visualParentNode.expanded then

                    visualNode:SetPoint("TOPLEFT", wrapper, "TOPLEFT", x + wrapper.indernalDeltaX + (visualNode.nodeData.dX or 0), -currentYOffset - 2 + wrapper.internalDeltaY
                     + (visualNode.nodeData.dY or 0))
                    
                    local dY = 20
                    
                    if visualNode.nodeData.textAddon then
                        dY = 28
                    end
                    
                    if visualNode.nodeData.extraSpace then
                        dY = dY + visualNode.nodeData.extraSpace
                    end

                    localYOffset = localYOffset + dY
                    currentYOffset = currentYOffset + dY
                    visualNode:Show()
                    visualNode.Button:ClearAllPoints()
                    ...
                    if visualNode.nodeData.textAddon then
                        visualNode.Button.highlight:SetHeight(30)
                        visualNode.Button:SetHeight(28)
                    else
                        visualNode.Button.highlight:SetHeight(dY)
                        visualNode.Button:SetHeight(dY)
                    end
                else
                    visualNode:Hide()
                end
                ...
                if visualNode.nodeData.textColor then
                    ...
                    visualNode.Button.Text:SetTextColor(color.r, color.g, color.b)
                end
                ...
                if visualNode.visualNodes and visualNode.expanded then
                    ...
                    currentYOffset = currentYOffset + (visualNode.nodeData.extraBottomGap or 0)
    ...
    function wrapper:UpdateTreeVisualization(isInThread)
        if self.visualNodes then
            for _, visualNode in pairs(treeVisualizationContainer[self.treePrefix]) do
                visualNode:Hide()
                visualNode:ClearAllPoints()
            end
    ...
    function wrapper:SaveExpansionState(stateName)
        ...
        for index, visualNode in pairs(treeVisualizationContainer[self.treePrefix]) do
...
function GUIUtils.SetScrollableTreeFrame(config)
...
function GUIUtils:MakeColorPicker(checkox, initialColor, changedCallback )
	if not checkox.colorInitialized then
        ...
        frame.checkox = checkox
		
		frame:SetScript("OnMouseDown", function(self_)
            local checkox_ = self_.checkox
			local r, g, b = unpack(checkox_:GetColor()) 
            local alpha = checkox_.alpha
			
			GUIUtils.isShowing = true
            GUIUtils:ShowColorPicker(r, g, b, alpha or 1, function()
				if not GUIUtils.isShowing then
					local r, g, b = ColorPickerFrame:GetColorRGB()
                    local alpha_ = 1 - OpacitySliderFrame:GetValue()

					checkox_:SetColor({r, g, b})
                    checkox_.alpha = alpha_
					
					if changedCallback then
						changedCallback(r, g, b, alpha_)
					end
				end
			end)
    ...
    if color[4] then
        checkox.alpha = color[4]
    end

    checkox:SetColor({color[1], color[2], color[3]})
end
...
function GUIUtils:ShowBindings(categoryName)
    ...
    for k, button in pairs(KeyBindingFrame.categoryList.buttons) do 
...
function GUIUtils:ShowColorPicker(r, g, b, alpha, changedCallback)
	ColorPickerFrame:SetColorRGB(r,g,b);	
	ColorPickerFrame.hasOpacity, ColorPickerFrame.opacity = (alpha ~= nil), 1 - alpha;
	ColorPickerFrame.previousValues = {r,g,b,alpha};
	ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
	changedCallback, changedCallback, changedCallback;
	ColorPickerFrame:Hide(); 
	ColorPickerFrame:Show();
    OpacitySliderFrame:SetValue(1 - alpha);
end
...
function GUIUtils:GetTaskPOI(index, buttonsBuffer, parent)
    local button = buttonsBuffer[index]

    while #buttonsBuffer < index do
        ...
        buttonsBuffer[#buttonsBuffer + 1] = button
    end
...
function Math:CreatePoint(x, y)
    ...
    function Point:DistanceToLine(line)
        local A = -line.a
        local C = -line.b
        local lengthAB = math.sqrt(A * A + 2)

        if lengthAB ~= 0 then
            local dist = math.abs((A * self.x + self.y + C)) / lengthAB;
            return dist
        else
            return 0
        end
...
function Math:CreateLine()
    --a 
    --b
    --P1
    --P2
    local Line = {}
    function Line:IntersectWith(other)
        local b1 = self.b
        local b2 = other.b
        ...
        local res =  Math:CreatePoint(x, y)
        return res;
    end
...
function Math:LineFromPoints(P1, P2)
    local result = Math:CreateLine()
    result.P1 = P1;
    result.P2 = P2;
...
---Segment:
function Math:CreateSegment(P1, P2)
    --P1 
    --P2    
    local Segment = {}
    ...
    function Segment:ToLine()
        return Math:LineFromPoints(self.P1, self.P2);
    end

    function Segment:IntersectWith(line)
        local line_ = self:ToLine()
        return line_:IntersectWith(line);
    end

    function Segment:IntersectWithAllowOnlyIn(line)
        local point = self:IntersectWith(line);

        if point then
            local distToP1 = point:DistanceTo(self.P1)
            local distToP2 = point:DistanceTo(self.P2)
            local maxDistance = self:Length()

            if distToP1 <= maxDistance and distToP2 <= maxDistance then
                return point
            end
        end
    ...
    function Segment:IntersectWithOther(segment)
        local point = self:IntersectWithAllowOnlyIn(segment:ToLine())

        if point then
            local distToP1 = point:DistanceTo(segment.P1)
            local distToP2 = point:DistanceTo(segment.P2)
            local maxDistance = segment:Length()
...
--Returns: x, y or nil if points is empty
function Math:Points2Center(points)
    local xSum, ySum = 0, 0
    local count = #points
...
function Math:ClosestPoint(point, points)
    local closest
    local closestIndex
    local smallestDistance

    for i = 1, #points do
        local point_ = Math:CreatePoint(points[i].x, points[i].y)
        
        if point_.x ~= point.x and point_.y ~= point.y then
            local distance = point_:DistanceTo(point)  
            if closest == nil or distance < smallestDistance then
                closest = point_
    ...
    return closest, closestIndex, smallestDistance
end

-- Jarvis' Gift wrapping algorithm.
function Math:CalculateArea(p, q, r)
    local cross = (q.y - p.y) * (r.x - q.x)
                - (q.x - p.x) * (r.y - q.y)
    return cross
end
  
function Math:IsOrientedCounterClockwise(p, q, r) 
    return Math:CalculateArea(p, q, r) < 0 
end

function Math:EnlargePoints(points, enlargeAmount)
    local points = LuaUtils:clone(points) or {}
    if enlargeAmount == 0 then
        return points
    end
    
    local cX, cY = Math:Points2Center(points)

    for _, point in pairs(points) do 
        local v = Math:CreatePoint(point.x - cX, point.y - cY)
        local length = v:DistanceTo(Math:CreatePoint(0,0))
		if length == 0 then length = 1 end -- need to check this
        local vNormal = Math:CreatePoint(v.x / length, v.y / length)

        local dX = vNormal.x * enlargeAmount
        local dY = vNormal.y * enlargeAmount
...
--[[ Tries to get subsets of points (that are far away from each other) if possible. 
              .                             . 
             .. .  .  .                  . .. ..   .
         .  .  subset1  .  .          .  .  subset2 . .  
             .  ..  .                   . .  .. .  . 
...
Result: {{subset1}, {subset2}, ...}}  If no subsets are found then it returns {{inputAllPoints}} 
Arguments: points - list of objects with property x and y 
]]
local Points2PointsGroups_cache = {}
function Math:Points2PointsGroups(points, enlargeAmount)
    local cacheKey = #points

    for _, point in pairs(points) do
        cacheKey = cacheKey + point.x + point.y
    end

    --Checking of Points2PointsGroups_cache isn't too big
    if LuaUtils:TableSize(Points2PointsGroups_cache) > 10 then
        Points2PointsGroups_cache = {}
    end

    local cachedValue = Points2PointsGroups_cache[cacheKey]
    if cachedValue then return  cachedValue end

    enlargeAmount = enlargeAmount or 0.01
    local points = LuaUtils:clone(points) or {}

    --point 2 true
    local groupA = {}
    local groupB = {}
    local x, y = Math:Points2Center(points)
    local centerPoint = Math:CreatePoint(x, y)

    if x and y then
        local step = 3.14159265359/8
        --Lines from 0 to 180 deg with step "step"
        for angle = 0, 3.14159265359, step do 
            groupA = {}
            groupB = {}
            ...
            local P2 = Math:CreatePoint(x + dX, y + dY)
            local line = Math:LineFromPoints(centerPoint, P2)

            for i=1, #points do
                local point = points[i]
                point = Math:CreatePoint(point.x, point.y)

                local dist = point:DistanceToLine(line)

                if math.abs(dist) > 0.05 then
                    --Checking the side
                    local crossY = line.a * point.x + line.b

                    if (crossY - point.y)>0 then 
                        groupA[#groupA + 1] = point
                    else
                        groupB[#groupB + 1] = point
                    end
                end
            end

            if (#groupA + #groupB) == #points and not(#groupA == 0 or #groupB == 0)  then
                cachedValue  = {Math:EnlargePoints(groupA, enlargeAmount), Math:EnlargePoints(groupB, enlargeAmount)}
                Points2PointsGroups_cache[cacheKey]= cachedValue
                return cachedValue
            end
        end
    ...
    cachedValue  = {Math:EnlargePoints(points, enlargeAmount)}
    Points2PointsGroups_cache[cacheKey]= cachedValue
    return cachedValue
end
...
function Math:Points2Shape(points)
    ...
    repeat
        ...
        for i = 1, numPoints, 1 do
            if Math:IsOrientedCounterClockwise(points[p], points[i], points[q]) then 
                q = i 
            end
        end
...
function Math:Points2ShapeSegments(points)
    local shape = Math:Points2Shape(points)
    local result = {}
    ...
    if shape then
        for i=1, #shape do
            local p1, p2
            if i == #shape then
                p1, p2 = shape[i], shape[1]
            else
                p1, p2 = shape[i], shape[i + 1]
            end

            result[#result + 1] = Math:CreateSegment(Math:CreatePoint(p1.x, p1.y), Math:CreatePoint(p2.x, p2.y))
        end
    end
...
function Math:IsPointInShape(point, segments)
    local intersectionAmount = 0

    local x = 100000000
    local y = point.y
    local segmentToRight = Math:CreateSegment(point, Math:CreatePoint(x, y))
   
    for k, seg in pairs(segments) do 
        if segmentToRight:IntersectWithOther(seg) then
            intersectionAmount = intersectionAmount + 1
        end
    end
...
-------------- Drawing -------------------
GUIUtils.Drawing = {}
local Drawing = GUIUtils.Drawing

local shapeName2Info = {}
local lineBuffers = {}
function Drawing:ClearShape(shapeName)
    
    local buffer = lineBuffers[shapeName]
    if buffer then

        if shapeName2Info[shapeName] then
            shapeName2Info[shapeName].visible = false
        end

        for k, line in pairs(buffer) do
            line:Hide()
        end
    end

    --Hidding border
    LuaUtils:HideLine("DrawPointsCould"..shapeName)
end

function Drawing:ClearAllShapes()
    for shapeName in pairs(lineBuffers) do
        Drawing:ClearShape(shapeName)
    end
end

function Drawing:DrawHLine(shapeName, frame, x1, x2, y, index, color)
   lineBuffers[shapeName] = lineBuffers[shapeName] or {}
   local buffer = lineBuffers[shapeName]

   local line = buffer[index] 
   if not line then 
        line = frame:CreateTexture(nil, "BACKGROUND", nil, -7)
        buffer[index] = line
   end

   line:ClearAllPoints()
   line:SetPoint("TOPLEFT", frame, x1, y)
   line:SetSize(x2-x1, 1)
   line:SetColorTexture(unpack(color or {0.5, 0.5, 1, 0.3}))
   line:Show()
   line:SetVertTile(true)
   line:SetHorizTile(true)

   return line
end

function Math:IsPointInShapeByName(point, shapeName) 
    local segments = shapeName2Info[shapeName].segments
    return Math:IsPointInShape(point, segments)
end

--returns new position. Adds new(changed or unchanged) circle position to tempTable
function Math:AvoidCircleOverlaping(initialX, initialY, currentId, tempTable) 
    local minDistance = 0.023
    for id, point in pairs(tempTable) do
        if currentId ~= id then

            local dX, dY = point.x - initialX, point.y - initialY
            local distance = math.sqrt(dX * dX + dY * dY)
            if distance < minDistance then
                local done = false

                local step = 6.28318/32
                for angle = 0, 6.28318, step do 
                    if not done then
                        local shiftX = math.sin(angle) * minDistance
                        local shiftY = math.cos(angle) * minDistance
            
                        local newX = initialX + shiftX
                        local newY = initialY + shiftY
                        local newDX, newDY = point.x - newX, point.y - newY
                        local newDistance = math.sqrt(newDX * newDX + newDY * newDY)
    ...
    tempTable[currentId] = {x=initialX, y=initialY}
    return initialX, initialY
end


local FillPolygon_cache = {}
function Drawing:FillPolygon(shapeName, segments, frame, color, onEnter, onLeave, cacheKeyPrefix)

    if LuaUtils:TableSize(FillPolygon_cache) > 20 then
        FillPolygon_cache = {}
    end

    Drawing:ClearShape(shapeName)

    shapeName2Info[shapeName] = shapeName2Info[shapeName] or {}
    local info = shapeName2Info[shapeName]
    ...
    for k, seg in pairs(segments) do
        cacheKey = cacheKey + seg.P1.x + seg.P1.y + seg.P2.x + seg.P2.y
    end

    cacheKey = tostring(cacheKeyPrefix or "") .. cacheKey

    local cachedArgs = FillPolygon_cache[cacheKey]
    if cachedArgs then
        for _, args in pairs(cachedArgs) do
            Drawing:DrawHLine(unpack(args))
        end
    ...
    --finding boundries
    for k, seg in pairs(segments) do 
        if not minX or seg.P1.x < minX then minX = seg.P1.x end
        if not minX or seg.P2.x < minX then minX = seg.P2.x end
        if not maxX or seg.P1.x > maxX then maxX = seg.P1.x end
        if not maxX or seg.P2.x > maxX then maxX = seg.P2.x end

        if not minY or seg.P1.y < minY then minY = seg.P1.y end
        if not minY or seg.P2.y < minY then minY = seg.P2.y end
        if not maxY or seg.P1.y > maxY then maxY = seg.P1.y end
        if not maxY or seg.P2.y > maxY then maxY = seg.P2.y end
    end
    ...
    for y = minY - 2 * step, maxY + 2 * step, 1/frameH do 
        local horizontalSegmnt = Math:CreateSegment(Math:CreatePoint(minX ,y), Math:CreatePoint(maxX ,y))
        local horizontalLine = horizontalSegmnt:ToLine()

        local ends = {}
        for k, seg in pairs(segments) do 
            local crossPoint = seg:IntersectWithAllowOnlyIn(horizontalLine)
            if crossPoint then
                ends[#ends + 1] = crossPoint
            end
        end

        if #ends == 2 then
            local args = {shapeName, frame, ends[1].x * frameW, ends[2].x * frameW, -y * frameH + 0.5, index, color}
            Drawing:DrawHLine(unpack(args))
            cachedArgs[#cachedArgs + 1] = args
            index = index + 1
        end
    end

    FillPolygon_cache[cacheKey] = cachedArgs
end

--Points - set of points with normalized coordinates
function Drawing:DrawPointsCould(shapeName, points, frame, colorr, onEnter, onLeave, drawBorder, cacheKeyPrefix)
    local shape = Math:Points2Shape(points)

    GUIUtils.Drawing:ClearShape(shapeName)

    if shape then
        local allSegments = Math:Points2ShapeSegments(points)
        Drawing:FillPolygon(shapeName, allSegments, frame, colorr, onEnter, onLeave, cacheKeyPrefix)

        if drawBorder then
            LuaUtils.StartChangingLinesPositions({"DrawPointsCould"..shapeName})

            for _, segment in pairs(allSegments) do 
                local w = DGV.DugisArrow.map_overlay:GetWidth()
                local h = DGV.DugisArrow.map_overlay:GetHeight()
                local visualLine = LuaUtils:GetNextVisualLine("DrawPointsCould"..shapeName, DGV.DugisArrow.map_overlay, "Interface/AddOns/DugisGuideViewerZ/Artwork/bulb_border.tga")
                LuaUtils:DrawLineDugi(visualLine, DGV.DugisArrow.map_overlay, segment.P1.x*w, -segment.P1.y*h, segment.P2.x*w, -segment.P2.y*h
                                        , 8 , 32/30, "TOPLEFT")
                visualLine:Show()
                visualLine:SetVertexColor(0.7, 0.7, 1, 0.4 )
            end

            LuaUtils:StopChangingLinesPositions({"DrawPointsCould"..shapeName})
        end
    end
end

function Drawing:drawTestShape()
    local points = { }
    ...
    Drawing:DrawPointsCould("UIParent", points, UIParent, {0.5, 0.5, 1, 0.5})
end


-----------------------Quests------------------------------
GUIUtils.Quests = {}
local Quests = GUIUtils.Quests

function Quests:SetupQuestSize(pin,  sizeA, sizeB)
    local readyForTurnIn = DGV.IsQuestReadyForTurnIn(pin.questId)
    sizeA = sizeA or 75
    sizeB = sizeB or 85
	pin.Texture:SetSize(sizeA, sizeA);
	pin.PushedTexture:SetSize(sizeA, sizeA);
	pin.Highlight:SetSize(sizeA, sizeA);
	pin.Number:SetSize(sizeB, sizeB);
    pin.Number:SetPoint("CENTER", 0, 0)

    if readyForTurnIn then
        local w = sizeB * 0.7
        pin.Number:SetSize(w, sizeB * 0.7)
        
    else
        local w = sizeB * 0.8
        pin.Number:SetSize(w, sizeB * 0.8)
    end
    
    pin:SetSize(sizeA, sizeA)
end


local GetQuestNumber_cache = {}
local function GetQuestNumber(questId)
    ...
    for i, questInfo in pairs(questsOnMap) do
        ...
        if questId == questInfo.questId then
            GetQuestNumber_cache[questId] = number
            return number
        end

        local readyForTurnIn = DGV.IsQuestReadyForTurnIn(questInfo.questId)

        if not readyForTurnIn then
            number = number + 1
        end
    end
        
    return GetQuestNumber_cache[questId] or number
end

function Quests:SetupQuestButton(pin, isSuperTracked, sizeA, sizeB, color)
    ...
    local numberIcon = "Interface/WorldMap/UI-QuestPoi-NumberIcons"

    if readyForTurnIn then
        numberIcon = "Interface/WorldMap/UI-WorldMap-QuestIcon" --[[Interface\AddOns\DugisGuideViewerZ\Artwork\turnin]]
    end
	...
	pin.Number:SetTexture(numberIcon);	
	pin.Number:SetTexCoord(0, 1, 0, 1)
    pin.Number:ClearAllPoints()
    
    if readyForTurnIn then
        local dX = 0.038
        pin.Number:SetTexCoord(0 + dX, 0.5 + dX, 0.5, 1)
        pin.Number:Show()
    else
        if pin.questId and pin.questId ~= 0 then
            pin.Number:SetTexCoord(GUIUtils.Quests:CalculateNumericTexCoords(GetQuestNumber(pin.questId), color));
            pin.Number:Show()
        else
            pin.Number:Hide()
        end
    ...
    Quests:SetupQuestSize(pin,  sizeA, sizeB, readyForTurnIn)

    return GetQuestNumber(pin.questId)

end

function Quests:CalculateNumericTexCoords(index, color_)
	local QUEST_POI_ICONS_PER_ROW = 8
	local QUEST_POI_ICON_SIZE = 256/QUEST_POI_ICONS_PER_ROW
	local color = QUEST_POI_ICON_SIZE * 4;

	if color_ == "yellow" then
		color = 0
	end
...
GUIUtils.OnFrameUpdate = function()
    for shapeName, shapeInfo in pairs(shapeName2Info) do
        local x, y = WorldMapFrame:GetNormalizedCursorPosition()
        if shapeInfo.visible then
            if WorldMapFrame:IsShown() and (not shapeInfo.mouseOver) and Math:IsPointInShapeByName(Math:CreatePoint(x, y), shapeName) then
                shapeInfo.mouseOver = true
                if shapeInfo.onEnter then
                    shapeInfo.onEnter(shapeInfo)
                end
            end

            if (shapeInfo.mouseOver) and (not WorldMapFrame:IsShown() or not Math:IsPointInShapeByName(Math:CreatePoint(x, y), shapeName)) then
                shapeInfo.mouseOver = false
                if shapeInfo.onLeave then
                    shapeInfo.onLeave(shapeInfo)
                end
            end
        else
            --In case shape whas removed/hidden
            if shapeInfo.mouseOver then
                shapeInfo.mouseOver = false
                if shapeInfo.onLeave then
                    shapeInfo.onLeave(shapeInfo)
                end
...
GUIUtils.getFRIZQTFont = function()
    if LOCALE_koKR then
...
GUIUtils.setCreature = function(frame, modelId)
   frame:SetCreature(modelId)
