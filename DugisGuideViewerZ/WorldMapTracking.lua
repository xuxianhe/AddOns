...
function DugisQuestsDataProvider:AddQuest(questId, x, y, frameLevelOffset, targetMapId)
	...
	GUIUtils.Quests:SetupQuestButton(pin, isSuperTracked, 70, 70, isSuperTracked and "yellow" or "brown")
...
function QuestPinMixin:OnLoad(questNumber)
    self:SetScalingLimits(1, 0.412, 0.412);
	self.UpdateTooltip = self.OnMouseEnter;
end

function QuestPinMixin:IsAttachedToQuest(questID)
...
function QuestPinMixin:OnMouseEnter()
	ShowQuestTooltip(self.questID or self.questId, DGV.IsQuestReadyForTurnIn(self.questID  or self.questId))
end
...
function QuestPinMixin:OnClick(button, b)
	DGV.OnQuestPOIClick(self.questID or self.questId)
	UpdateTrackingFilters(true)
end
...
function WMT:Initialize()
	...
	local getTrackingInfoMap = {
		[1] = {"Auctioneer", 136452 },
		[2] = {"Banker", 136453 },
		[3] = {"Battlemaster", 136454 },
		[4] = {L["Class Trainer"], [[Interface\MINIMAP\TRACKING\Class]]},
		[5] = {"Flight Master", 136456 },
		[6] = {"Food & Drink", 136457 },
		[7] = {"Innkeeper", 136458 },
		[8] = {"Mailbox", 136459 },
		[9] = {"Poisons", 136462 },
		[10] = {"Profession Trainers", 136463 },
		[11] = {"Reagents", 136464 },
		[12] = {"Repair", 136465 },
		[14] = {"Quests", [[Interface\MINIMAP\TRACKING\QuestBlob]] },
		[15] = {"Quests Objectives", [[Interface\AddOns\DugisGuideViewerZ\Artwork\bullet_white]] },
		[16] = {"Spirit Healer", [[Interface\WORLDSTATEFRAME\ColumnIcon-GraveyardDefend1]] },
		[17] = {"New Quests", [[Interface\AddOns\DugisGuideViewerZ\Artwork\accept]] }
	}
	local function GetTrackingInfo(id)
		if not getTrackingInfoMap[id] then
DGV:DebugFormat("GetTrackingInfo fail", "id", id)
		end
		return unpack(getTrackingInfoMap[id])
	end
		...
		for _,point in pairs(trackingPoints) do
			...
			then
				...
				if not point:IsShown() then
					...
					if not tonumber(point.mapId) then
						local mapName,level = strsplit(":",point.mapId)
						...
						point.mapId = DGV:GetMapIDFromShortName(nsMapName or mapName)
...
--WorldMapFrame.IsMaximized doesn't exist. INFO: in classic the map is always maximized		
    ...
    DugisWaypointTooltip.updateModel = function()
		...
		if mv and mv.npcDB and mv.npcDB[npcId] then
			...
			if value and value ~= "" then
				GUIUtils.setCreature(modelFrame, npcId or value)
			end
		else
	...
	local function IterateZonePoints(mapName, pointData, ofType, allContinents, IterateZonePoints, dontCheckDistance)
		...
		if 
			DGV.GetCurrentMapContinent_dugi()~=DGV.GetMapContinent_dugi(mapID) and
			not allContinents
		then
			return
		end
...
--DGV:DebugFormat("IterateZonePoints", "mapName", mapName, "ShouldShow", (DataProviders:ShouldShow(nil, point[3], point[4], unpack(point, 5))), "GetDistance", (GetDistance(point)))
	...
	function WMT:GetQuestPosition(questId)
		...
		if DGV.IsQuestReadyForTurnIn(questId) then
			local questInfo_ = quests[tonumber(questId)]
			...
			for point in DGV.IterateAllFindNearestPoints() do
                ...
                if name and not added[name] then
                    ...
                    if trackingType==10 then
						...
						for point1 in DGV.IterateAllFindNearestPoints() do
							DGV.YieldAutoroutine()
        ...
        if ElvUIMiniMapTrackingDropDown then
            hooksecurefunc(ElvUIMiniMapTrackingDropDown, "initialize", function()
				LuaUtils:Delay(0.01, function()
					DGV.BeginAutoroutine("ShowExtraMenu", ShowExtraMenu)
				end)
            end)
